...
export default function AdminVoiceProxy() {
  ...
  const startCall = useCallback(async () => {
    ...
    try {
      ...
      setCallState({ status: 'ringing', callSid: data.call_sid, conferenceName: data.conference_name, targetNumber: phoneNumber });
      setMessages([]);
  ...
  const sendText = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${SUPABASE_URL}/functions/v1/voice-proxy-speak`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` },
        body: JSON.stringify({ text, voice_id: voiceId, conference_name: callState.conferenceName }),
      });
      const data = await res.json();
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="text-center mb-6">
        <h2 className="text-2xl font-bold mb-1">Voice Proxy</h2>
        <p className="text-muted-foreground text-sm">Call anyone and speak through an AI-generated voice</p>
      </div>
      ...
      {callState.status === 'idle' || callState.status === 'ended' ? (
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              <label className="text-sm font-medium mb-1 block">Target Phone Number</label>
              <Input type="tel" placeholder="+1 (555) 123-4567" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} />
            </div>
            <div>
              <label className="text-sm font-medium mb-1 block">Your Phone (to listen in)</label>
              <Input type="tel" placeholder="+1 (555) 987-6543" value={listenerPhone} onChange={(e) => setListenerPhone(e.target.value)} />
        ...
        <>
            ...
            <CardContent className="py-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-3 h-3 rounded-full ${callState.status === 'connected' ? 'bg-primary' : 'bg-accent animate-pulse'}`} />
                  <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button onClick={endCall} variant="destructive" size="sm"><PhoneOff className="w-4 h-4" /></Button>
                </div>
              </div>
            </CardContent>
          </Card>
              ...
              <div className="min-h-[200px] max-h-[400px] overflow-y-auto space-y-2">
                {messages.length === 0 ? (
                  <p className="text-center text-muted-foreground text-sm py-8">
                    {callState.status === 'connected' ? "Type below to speak through your chosen voice..." : "Waiting for the other party to pick up..."}
                  </p>
                ) : (
                  messages.map((msg, i) => (
                    <div key={i} className="flex items-start gap-2 ml-auto max-w-[80%] justify-end">
                      <div className={`px-3 py-2 rounded-lg text-sm ${msg.status === 'error' ? 'bg-destructive/10 text-destructive' : msg.status === 'sending' ? 'bg-muted opacity-70' : 'bg-primary/10'}`}>
                        <p>{msg.text}</p>
          ...
          <div className="flex gap-2">
            <Input
              ...
              className="flex-1"
            />
            <Button onClick={sendText} disabled={callState.status !== 'connected' || !textInput.trim() || isSending} size="icon">
              <Send className="w-4 h-4" />
            </Button>
          </div>
          <p className="text-xs text-center text-muted-foreground mt-4">Type your message and press Enter. ~1 second delay between typing and playback.</p>
        </>
      )}
    </div>
  );
