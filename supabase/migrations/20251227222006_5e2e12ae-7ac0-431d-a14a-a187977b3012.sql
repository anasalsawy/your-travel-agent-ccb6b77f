-- Drop existing notification triggers to recreate them
DROP TRIGGER IF EXISTS on_order_created ON public.orders;
DROP TRIGGER IF EXISTS on_payment_proof_uploaded ON public.orders;
DROP TRIGGER IF EXISTS notify_order_update ON public.orders;
DROP TRIGGER IF EXISTS on_ticket_request_created ON public.ticket_requests;
DROP TRIGGER IF EXISTS notify_ticket_request_update ON public.ticket_requests;

-- Create comprehensive notification trigger function
...
-- Create triggers for ticket_requests
CREATE TRIGGER on_ticket_request_insert
  AFTER INSERT ON public.ticket_requests
  FOR EACH ROW
  EXECUTE FUNCTION trigger_notification();

CREATE TRIGGER on_ticket_request_update
  AFTER UPDATE ON public.ticket_requests
  FOR EACH ROW
  EXECUTE FUNCTION trigger_notification();